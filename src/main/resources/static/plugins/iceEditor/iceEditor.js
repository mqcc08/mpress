...
ice.editor.prototype={
	...
	extendSetHTML:function(html,a){
		html = this.extendParseElement(html);
		this.element.focus();
		var range = this.range.getRangeAt(0);
		//将选中的文档放在html中的DOM内
		if(!a)html.appendChild(range.extractContents());
	...
	extendParseElement(html){
		var template = "<div class='child'>"+html+"</div>";
		var cdiv = this.c("div");
		cdiv.innerHTML = template;
		return cdiv.firstChild;
	},
	//插入HTML
	extendAppendHtml:function(html ,a){
		this.element.focus();
		var range = this.range.getRangeAt(0);
		//将选中的文档放在html中的DOM内
		if(!a)html.appendChild(range.extractContents());
		...
		//创建文档碎片并放入新节点
		//range.innerHTML()
		range.innerHTML(this.w.document.createDocumentFragment().append(html));//合并范围至末尾
		//合并范围至末尾
		range.collapse(false);
	},
	//插入文字内容
	...
	//创建菜单
	extendCreateMenu:function(json){
		var _z = this;
		var li = this.c('li');
		if(json.id)li.id = json.id;
		if(json.css)li.className = json.css;
		...
		if(json.menu || json.icon){
			var div = this.c('div');
			var iconDiv = this.c('div');
			iconDiv.setAttribute("class" , json.icon)
			if(json.title)div.title = json.title;
			div.className="iceEditor-exec extend";
			if(json.data)div.setAttribute('data',json.data);
	...
	//插件开发
	extendPlugin:function(json){
		if(json.name == undefined) return console.log('plugin：menu参数不能为空');
		if(this.inArray(json.name,this.menu)) return console.log('plugin：menu已经存在，请重新命名');
		this.menu.push(json.name);
		this.extendCreateMenu(json);
	},
};
//工具栏菜单HTML
